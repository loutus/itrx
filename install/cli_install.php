...
function setup_db($data) {
	...
	if ($lines) {
		...
		$db->query("INSERT INTO `" . $data['db_prefix'] . "user` SET user_id = '1', user_group_id = '1', username = '" . $db->escape($data['username']) . "', salt = '" . $db->escape($salt = token(9)) . "', password = '" . $db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = 'Admin', lastname = 'Site', email = '" . $db->escape($data['email']) . "', status = '1', date_added = NOW()");
